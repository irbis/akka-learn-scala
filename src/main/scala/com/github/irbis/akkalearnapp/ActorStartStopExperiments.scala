...
object MainActorStartStop {
  def apply(): Behavior[String] =
    Behaviors.setup(new MainActorStartStop(_))
}

class MainActorStartStop(context: ActorContext[String]) extends AbstractBehavior[String](context) {
  override def onMessage(msg: String): Behavior[String] =
    msg match {
      case "start" =>
...
object ActorStartStopExperiments extends App {
  val actorStartStopSystem = ActorSystem(MainActorStartStop(), "actorStartStopSystem")
  actorStartStopSystem ! "start"
}